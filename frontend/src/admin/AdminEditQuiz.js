...
export default function AdminEditQuiz() {
  ...
  return h(
    ...
    [
      ...
      h('section', { className: 'rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-5' }, [
        h('div', { className: 'mb-4 font-medium' }, 'Pertanyaan'),
        h(
          'div',
          { className: 'space-y-6' },
          questions.map((q, idx) =>
      ...
      h('div', { className: 'grid grid-cols-2 gap-4' }, [
        h('button', { onClick: () => navigate('/admin/manage'), className: 'h-11 rounded-xl border border-[var(--color-border)] bg-white' }, 'Batal'),
        h('button', { onClick: saveQuiz, className: 'h-11 rounded-xl bg-[var(--color-primary)] text-white' }, 'Perbarui Kuis'),
      ]),
    ]
  )
}
